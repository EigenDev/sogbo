    ...
    */ 
    constexpr units::emissivity calc_emissivity(const units::mag_field bfield, const units::ndens n, const double p)
    {
        return (
            ...
            units::math::pow<std::ratio<3>>(constants::e_charge) / (constants::m_e * constants::c_light * constants::c_light) * n * bfield
        );
    }
        ...
        {       
            ...
            for (int jj = 0; jj < nj; jj++)
            {
                ...
                for (int ii = 0; ii < ni; ii++)
                {
                    ...
                    const auto d             = 1e28 * units::cm;                                                                       // distance to source
                    ...
                    const auto dvolume  = dx3 * dcos * (1.0 / 3.0) * (x1r * x1r * x1r - x1l * x1l * x1l) * qscales.length_scale * qscales.length_scale * qscales.length_scale * units::cm3;      

                    // observer time
                    const auto t_obs = t_prime - x1[ii] * qscales.length_scale * vector_dotproduct(rhat, obs_hat) * units::cm / constants::c_light;
                    ...
                    const units::frequency nu_c         = calc_nu(gamma_crit, nu_g);                                   // Critical frequency
                    const units::frequency nu_m         = calc_nu(gamma_min, nu_g);                                    // Minimum frequency
