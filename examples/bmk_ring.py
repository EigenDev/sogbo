...
def main():
    ...
    # Initial arrays
    theta_left_wing_idx  = find_nearest(theta, np.pi/2 - args.theta_j)[0]
    theta_right_wing_idx = find_nearest(theta, np.pi/2 + args.theta_j)[0]
    ...
    for tidx, t in enumerate(times):
        ...
        smask        = (chi >= 1.0)
        # Northern jet
        rho[theta_left_wing_idx:theta_right_wing_idx + 1,   smask]       = calc_rho(gamma_shock[smask], chi[smask], rho0, args.k)
        gamma_fluid[theta_left_wing_idx:theta_right_wing_idx + 1, smask] = calc_gamma_fluid(gamma_shock[smask], chi[smask])
        pressure[theta_left_wing_idx:theta_right_wing_idx + 1, smask]    = calc_pressure(gamma_shock[smask], chi[smask], rho0, args.k)
